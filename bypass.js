  ...
  <div class="panel">
    ...
    function handleVolcano() {
        ...
        function autoCopyKey() {
            ...
            const copyButtons = document.querySelectorAll('#copy-key-btn, .copy-btn, [aria-label="Copy"], button[class*="copy"], button[onclick*="copy"]');

            for (const copyBtn of copyButtons) {
                if (copyBtn && copyBtn.offsetParent !== null) { // Check if button is visible
                    try {
                        copyBtn.click();
        ...
        function findKeyNearElement(element) {
            ...
            const container = element.closest('.flex, .container, .mx-auto, .text-center');
            if (container) {
                ...
                for (const element of elements) {
                    ...
                    if (text && text.length > 10 && /^[a-zA-Z0-9\-_=+]+$/.test(text)) {
                        ...
                        if (typeof GM_setClipboard !== 'undefined') {
                            ...
                            if (panel) panel.show('bypassSuccessCopy', 'success');
                            if (debug) console.log('[Debug] Key automatically copied to clipboard');
                            return true;
                        }
                    }
                }
        ...
        function actOnCheckpoint(node) {
            ...
            if (!copyButtonClicked) {
                ...
                if (success) {
                    ...
                    setInterval(() => {
                        const copyButtons = document.querySelectorAll('#copy-key-btn, .copy-btn, [aria-label="Copy"]');
                        for (const copyBtn of copyButtons) {
                            if (copyBtn && copyBtn.offsetParent !== null) {
                                try {
                                    ...
                                    if (debug) console.log('[Debug] Error in copy button spam', err);
                                }
                            }
                        }
                    }, 1000);
